...
import time
from modules import *
from modules.ui import compact_overlay_ui, loading_ui, main_tilda_ui, popup_window_ui
...
#mappc.main_application_controller(self)
...
class Window(QMainWindow, main_tilda_ui.Ui_MainWindow):
    ...
    def apply_and_restart(self):
        self.save_settings()
        self.init_main_refresh_activation()
    ...
    def app(self):
        ...
        self.cancel_button.clicked.connect(self.apply_settings)
        self.pushButton_6.clicked.connect(self.enable_smm_overlay)
        popup_w.pushButton.clicked.connect(self.exit_app)
                ...
                if type=="P":
                        ...
                        try:
                                ...
                                if loc and len(list(zip(*loc[::-1]))) > 0:
                                        ...
                                        header = pyautogui.screenshot(region=(pt[0], pt[1], 350*scale_aba, 30*scale_aba))
                                resultimg = cv2.cvtColor(numpy.array(header), cv2.COLOR_RGB2BGR)
                                import dbman
